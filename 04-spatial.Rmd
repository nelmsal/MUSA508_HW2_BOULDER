...
## 1. Spatially Visualizing Test Dataset & Training Model

### A. Map of Predicted Prices & Errors in Test Dataset 
...
Var3_map = var_cut_map(
  ...
  pal_rev = TRUE,
  var_breaks_nomax = c(min_me, -1000000, -250000, -50000, 50000, 250000,1000000),
  var_title = "Mean Errors",
  var_legend = 'Mean of Observed Price\nsubtracted by Predicted Price',
  var_num = 'C'
)
...
grid.arrange(
  ...
  Var4_map,
  ncol=2,
  top = glue('Map {map_num}: Home Value Predictions\nTraining Model on Test Dataset')
)
```
...
# grid.arrange(ncol=2,
#   ggplot(data = boulder.test.fishnet) +
#     geom_point(aes(x = avg.price, y = ME)) +
#     labs(title=title,
#       subtitle = "Price & ME") +
#     plotTheme(),
#   
#   ggplot(data = boulder.test.fishnet) +
#     geom_point(aes(x = avg.price, y = MAE)) +
#     labs(title='', subtitle = "Price & MAE") +
#     plotTheme()
# )

# ggplot(data = boulder.test.fishnet) +
#     geom_point(aes(x = MPE, y = avg.price)) +
#     labs(title='', subtitle = "Price & MAE") +
#     plotTheme()

# boulder.test %>%
#   mutate(PE = (price.abserror / price)) %>% 
#   filter(PE>3) %>% arrange(desc(PE))
