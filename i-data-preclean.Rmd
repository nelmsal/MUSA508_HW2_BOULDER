...
unique(boulder.ele.pts$Type_) %>% sort()
...
dist_rank = 
  stringdistmatrix(
    school.elementary.rank$school,
    boulder.ele.pts$School_Name
  )
boulder.sales %>% 
  merge(
    ., 
...
lapply(school.elementary.rank$school, function(rank_str) amatch(rank_str, boulder.ele.pts$School_Name))
...
boulder.sales$ele.nn.dist =
  nndist(
    ...
    boulder.ele.pts$geometry %>% as.ppp(.) # 172 cols
  )
...
# set_key("gyZXx_gohS8TYDgERoDYTgXlrpOhBek_S1UbuWlYp_8")
# ele_rank_geom = geocode(school.elementary.rank$address_full)
# ele_rank_geom

boulder.ele.pts$School_Coded = paste("x", boulder.ele.pts$School_Code %>% str_pad(., 4, pad = "0"), sep='')

school.elementary.rank$geometry = ele_rank_geom$geometry
school.elementary.rank = school.elementary.rank %>% st_sf(.) %>% st_set_crs(col_crs)
school.elementary.rank$geocode.add = ele_rank_geom$address

school.elementary.rank$ele.nn.id =
  st_nn_id_column(
    school.elementary.rank, # 11,364 rows
    boulder.ele.pts, # 172 cols
    focus_id = '',
    compare_id = 'School_Coded'
  )

school.elementary.rank$ele.nn.dist =
  nndist(
    school.elementary.rank$geometry %>% as.ppp(.), # 11,364 rows
    boulder.ele.pts$geometry %>% as.ppp(.) # 172 cols
  )
...
school.elementary.rank$district = 
          strsplit(school.elementary.rank$district, "District") %>% 
            as.data.frame() %>% 
            t %>% 
...
colnames(school.elementary.rank)
glimpse(school.elementary.rank %>% arrange(district_id))
glimpse(boulder.ele.pts %>% arrange(District_Code))
...
school.elementary.rank$geocode.add


school.elementary.rank$geocode.pct = 
...
setdiff(
  school.elementary.rank$school_id,
  boulder.sales$ele.nn.id
  
)

unique(boulder.sales$ele.nn.id) %>% sort(.)
unique(boulder.ele.pts$School_Code) %>% sort(.)
unique(school.elementary.rank$school_id) %>% sort(.)
...
'0436' %in% boulder.ele.pts$School_Code
fix
```
