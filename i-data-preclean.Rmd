...
get_max_yx_lengths = function(sf_focus){
  ...
  f_box = boulder.nhoods.focus %>% st_bbox(.)
...
get_fish_name = function(focus_name){
  ...
  return(boulder.nhoods.focus.fish.group)
}
...
boulder.nhoods.orient = 
  do.call(rbind.data.frame, 
        lapply(boulder.nhoods.focus.group$name, get_fish_name))
...
ggplot() + 
  ...
  #   color='transparent') + 
  geom_sf(
    data = boulder.nhoods.focus.fish %>%
      group_by(orientation) %>%
      summarize(geometry=st_union(geometry)), 
    fill='transparent',
    color='grey50') + 
  geom_sf(
    data = boulder.nhoods.focus.fish.group,
    aes(fill=nhood_id), alpha=.5,
    color='black', linetype = "dashed") +
  geom_sf(
    data = boulder.nhoods.focus.fish %>% st_union(.) %>% st_centroid(.), 
    color='black') +
  ...
  geom_text_sf(sf_to_labels(
    boulder.nhoods.focus.fish %>% 
      group_by(orientation) %>%
      summarize(geometry = st_union(geometry)), 
    'orientation'), color='grey50', size=1) +
  geom_text_sf(sf_to_labels(boulder.nhoods.focus.fish.group, 
    'neighborhood'), color='black') +
