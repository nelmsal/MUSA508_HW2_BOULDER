...
unique(col.dist.polys$school_level)

st_distance_dist_id = function(
  focus_sf, compare_sf,
  focus_id='', compare_id=''){
  
  if (focus_id==''){
    focus_sf$focus_id = row_number()
  }else
  {
    focus_sf$focus_id = focus_sf[[focus_id]]
  }
  
  if (compare_id==''){
    compare_sf$compare_id = row_number()
  }else
  {
    compare_sf$compare_id = compare_sf[[compare_id]]
  }
  ...
  names(dist_df) = compare_sf$compare_id
  rownames(dist_df) = focus_sf$focus_id
...
boulder.sales_ele =
  st_distance_dist_id(
    boulder.sales, # 11,364
    boulder.ele.pts,
    focus_id = 'MUSA_ID',
    compare_id = 'School_Code'
  )

dim(boulder.sales_ele)

boulder.sales$nn.ele = 
  matrixStats::rowMins(boulder.sales_ele)
boulder.sales$nn.ele_id = 
  matrixStats::rowMins(boulder.sales_ele, value = T)
