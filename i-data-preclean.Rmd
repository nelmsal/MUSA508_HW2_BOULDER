...
boulder.nhoods.cz =
  ...
  mutate_by(
    group=name, 
    name_count = n(),
    neighborhood = paste(name, row_number()))
...
boulder.nhoods.focus =
  boulder.nhoods.cz %>%
    filter(boulder.nhoods.cz[[focus_id_field]] == 'Lanyon') %>%
  mutate_by(
    group=zhood,
    group.geometry = st_union(geometry),
    group.centroid = st_centroid(geometry),
    group.area = st_centroid(geometry)
    ) %>%
...
ggplot() + 
  ...
  #   color='transparent') + 
  geom_sf(
    data = boulder.nhoods.group, 
    aes(fill=zhood), 
    color='grey50') + 
  geom_sf(
    data = boulder.nhoods.group %>% st_union(.) %>% st_centroid(.), 
    color='black') +
  ...
  geom_sf(data = boulder.county, fill = 'transparent', color='black',
          lwd=1) + 
  geom_text_sf(sf_to_labels(boulder.nhoods.group, 'zhood_id')) +
  theme(legend.position = "none") + 
  mapTheme() +
  plot_limits(poly.geometry = boulder.nhoods.group$geometry)
