...
select_v = function(sf, variable_names=c('price', boulder.dv)){
  return(sf %>% st_drop_geometry(.) %>%
             select(variable_names))}

select_dv = function(sf, variable_names=boulder.dv){
  return(sf %>% st_drop_geometry(.) %>%
             select(variable_names))}
...
inTrain = createDataPartition(
              y = do.call(paste, c(
                boulder.predict.0 %>% select_dv(.)
                , sep=" ")), 
              p = .75, list = FALSE)

boulder.training  = boulder.predict.0[inTrain,]
...
variable_list = boulder.dv %>% sort() %>% list()
variables_str = do.call(paste, c(boulder.dv %>% list(), collapse = "+"))

fm_equation = as.formula(paste(dependent_variable, variables_str, sep="~"))

print(fm_equation)

fit.training =lm(fm_equation, data = boulder.training %>% select_v(.))

stargazer(
  fit.training, type="text", digits=1, 
  title="Table 2: Boulder Training Data Regression Output", 
  out = "results/Training_LM.txt")

boulder.test <-
  boulder.test %>%
  mutate(Regression = " Boulder Test Regression",
...
fit.cv =
  train(price ~ .,
    data = boulder.training %>% select_v(.),
...
stargazer(fit.training, type="text", digits=1, title="Table 2: Boulder Training Data Regression Output", out = "Training LM.txt")
...
##This one was hard for me to figure out for a single test set, so I used a previous year's as a reference 
...
fit.cv.predict.0 <-
  ...
  mutate(Regression = "Baseline Regression",
         SalePrice.Predict  = predict(fit.cv, boulder.predict), 
         SalePrice.Error    = SalePrice.Predict - SalePrice, 
         SalePrice.AbsError = abs(SalePrice.Predict - SalePrice), 
         SalePrice.APE      = SalePrice.AbsError / SalePrice) %>%  
  filter(SalePrice < 5000000)
...
reg75 <- fit.cv.predict.0[fold75,c("SalePrice", "SalePrice.Predict")]
reg75.test <-
  reg75 %>%
...
fit.cv.rs.min$MAPE <- mean(reg75.test$SalePrice.APE)
...
predictedprices <- boulderData %>%
  mutate(prediction = predict(fit.cv, boulderData))
...
nhood.pricespredicted <- boulderdata %>%
  mutate(prediction = predict(reg.nhood, boulderdata))
...
reg.nhood <- lm(SalePrice ~ ., data = as.data.frame(boulder.training) %>% 
                                 dplyr::select(neighborhood, SalePrice, XXXXXXXXXXxx))

boulder.test.nhood <-
  boulder.test %>%
  mutate(Regression = "neighbors Effects",
         SalePrice.Predict = predict(reg.nhood, boulder.test), 
...
bothRegressions <-
  rbind(
    dplyr::select(boulder.test, starts_with("SalePrice"), Regression, neighborhood) %>%
      mutate(lagPriceError = lag.listw(spatialWeights.test, SalePrice.Error)),
    dplyr::select(boulder.test.nhood, starts_with("SalePrice"), Regression, neighborhood) %>%
      mutate(lagPriceError = lag.listw(spatialWeights.test, SalePrice.Error)))   
...
plot(mean_scatter_nhood$SalePrice.Predict, mean_scatter_nhood$SalePrice.APE, main="Figure XXX: MAPE by neighbors and Mean Price by neighbors", xlab="Mean Price by neighbors", ylab="MAPE by neighbors") +
  plotTheme()
