...
toplim=max(boulder.predict.0$price)
...
for (
  ...
  xlab(variable_name) +
  ylab("price") +
  ylim(min(boulder.predict.0$price), toplim) + 
  plotTheme()
}
...
ggplot(
    ...
    aes(
      ...
      y = boulder.predict.0$price)) +
  geom_point(size=2, shape=20) +
  labs(title = glue("Figure 2.{variable_number}: {variable_name}"),
       # subtitle = "XXXXXXXXXXXX"
       ) +
  geom_smooth(method = "lm", se=F,
              colour = plot_color) +
...
# #2 variable 
# 
# ggplot(data = boulder.predict.0, aes(x = PREDICTOR VARIABLE, y = SalePrice)) +
#   geom_point(size=2, shape=20)  +
